...
class StratifiedKFoldTrainer:
    ...
    def train_step(self, batch: Batch) -> torch.Tensor:
        ...
        metrics = self.train_metrics(logits.sigmoid().cpu(), batch.label_id.int())
    ...
    def validate_step(self, batch: Batch) -> torch.Tensor:
        ...
        self.val_metrics.update(logits.sigmoid().cpu(), batch.label_id.int())
        return val_loss

    def on_validate_end(self) -> None:
        """On validate end."""
        val_metrics = self.val_metrics.compute()
        self.cv_score.update(val_metrics["val_MultilabelAUROC"])
        wandb.log(
            {
