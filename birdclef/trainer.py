...
class ClassifierLightningModule(LightningModule):
    ...
    def training_step(self, batch: Batch, batch_idx: int) -> torch.Tensor:
        """Train step."""
        logits = self.model(batch.audio, None)
        ...
        accuracy = self.acc_fn(probs, labels)
        precision = self.pre_fn(probs, labels)
        recall = self.rec_fn(probs, labels)
        auroc = self.auroc_fn(probs, labels)
